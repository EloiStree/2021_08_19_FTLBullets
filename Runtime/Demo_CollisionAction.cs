...
public interface I_ToProcessCollisions {
    void GetBullets(out IEnumerable<ICapsuleLineWithId> bulletsAsCapsuleLine);
    void AppendBullets(IEnumerable<ICapsuleLineWithId> bullets);
    void RemoveBullet(ICapsuleLineWithId bullet);

    void GetBulletable(out ICapsuleLineWithId bulletableTargetAsCapsuleLine);
    void SetBulletable(ICapsuleLineWithId bulletable);
}

public class ToProcessBulletableCollisionsWithDico: I_ToProcessCollisions
{
    LinkedList<ICapsuleLineWithId> m_ListIn = new LinkedList<ICapsuleLineWithId>();
    Dictionary<string, ICapsuleLineWithId> m_bullets = new Dictionary<string, ICapsuleLineWithId>();
    ICapsuleLineWithId m_target;

    public void AppendBullets(IEnumerable<ICapsuleLineWithId> bullets)
    {
        foreach (ICapsuleLineWithId line in bullets)
        {
    ...
    public void RemoveBullet(ICapsuleLineWithId bullet)
    {
        bullet.GetUniqueId(out string id);
        m_bullets.Remove(id);
        m_ListIn.Remove(bullet);
    }
    public void GetBulletable(out ICapsuleLineWithId bulletableTargetAsCapsuleLine)
    {
        bulletableTargetAsCapsuleLine = m_target;
    }
    public void GetBullets(out IEnumerable<ICapsuleLineWithId> bulletsAsCapsuleLine)
    {
        bulletsAsCapsuleLine= m_ListIn;
    }
    public void SetBulletable(ICapsuleLineWithId bulletable)
    {
        m_target = bulletable;
    }
}
